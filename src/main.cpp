...
std::pair<Vec2d<double>, Vec2d<double>> loadMnist(const std::string& datasetPath, const std::string&& subdataset) {
    Vec2d<double> X;
    Vec2d<double> y;
    y.push_back({});
    for (const auto& clothingFolder : std::filesystem::directory_iterator(datasetPath + "/" + subdataset)) {
        if (std::filesystem::is_directory(clothingFolder.status())) {
            for (const auto& clothingImg : std::filesystem::directory_iterator(clothingFolder.path())) {
                cv::Mat readImg = cv::imread(clothingImg.path(), cv::IMREAD_GRAYSCALE);
                readImg = readImg.reshape(1, readImg.total() * readImg.channels());
                std::vector<double> imgVec = readImg.isContinuous() ? readImg : readImg.clone();
                X.push_back(imgVec);
                y[0].push_back(std::stod(clothingFolder.path().filename().string()));
            }
        }
    }
...
Vec2d<double> loadImage(const std::string& imagePath) {
    ...
    std::vector<double> imgVec = resizeImg.isContinuous() ? resizeImg : resizeImg.clone();
    Vec2d<double> img;
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: ./nnfs <MNist Directory> <Test Image>" << std::endl;
        return 1;
    }
    ...
    m.train(std::make_unique<Vec2d<double>>(xTrain), 
            ...
            std::make_unique<Vec2d<double>>(yTest), 
            10, 128);
