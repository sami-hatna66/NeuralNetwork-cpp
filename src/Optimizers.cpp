...
template <typename T>
void OptimizerBase<T>::finalize() {
    iterations += 1;
}

template <typename T>
T OptimizerBase<T>::getCurrentLearningRate() {
    return currentLearningRate;
}

template <typename T>
StochasticGradientDescent<T>::StochasticGradientDescent(T pLearningRate, T pDecay, T pMomentum) 
...
template <typename T>
void Adagrad<T>::updateParams(Layer<T>& layer) {
    layer.setWeightCache(layer.getWeightCache() + power<T>(layer.getDWeights(), 2.0));
    layer.setBiasCache(layer.getBiasCache() + power<T>(layer.getDBiases(), 2.0));
...
template <typename T>
void RMSprop<T>::updateParams(Layer<T>& layer) {
    layer.setWeightCache((rho * layer.getWeightCache()) + ((1 - rho) * power<T>(layer.getDWeights(), 2.0)));
    layer.setBiasCache((rho * layer.getBiasCache()) + ((1 - rho) * power<T>(layer.getDBiases(), 2.0)));
...
Adam<T>::Adam(T pLearningRate, T pDecay, T pEpsilon, T pBeta1, T pBeta2)
        : epsilon{pEpsilon}, beta1{pBeta1}, beta2{pBeta2}, OptimizerBase<T>{pLearningRate, pDecay} {}

template <typename T>
void Adam<T>::updateParams(Layer<T>& layer) {
    ...
    Vec2d<T> correctedWeightMomentums = layer.getWeightMomentums() / (1 - std::pow(beta1, iterations + 1));
    Vec2d<T> correctedBiasMomentums = layer.getBiasMomentums() / (1 - std::pow(beta1, iterations + 1));

    layer.setWeightCache(beta2 * layer.getWeightCache() + (1 - beta2) * power<T>(layer.getDWeights(), 2.0));
    layer.setBiasCache(beta2 * layer.getBiasCache() + (1 - beta2) * power<T>(layer.getDBiases(), 2.0));

    Vec2d<T> correctedWeightCache = layer.getWeightCache() / (1 - std::pow(beta2, iterations + 1));
    Vec2d<T> correctedBiasCache = layer.getBiasCache() / (1 - std::pow(beta2, iterations + 1));
