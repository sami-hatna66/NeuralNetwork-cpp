...
// FOR TESTING
template <typename T>
DenseLayer<T>::DenseLayer(int numInputs, int numNeurons, int num, T pWeightRegularizerL1,
...
template <typename T> Vec2d<T> &DenseLayer<T>::getDWeights() { return dWeights; }
...
template <typename T> Vec2d<T> &DenseLayer<T>::getBiasCache() { return biasCache; }
...
template <typename T> void DenseLayer<T>::setWeights(const Vec2d<T> &newWeights) {
    weights = newWeights;
}
...
template <typename T>
void DropoutLayer<T>::compute(const Vec2d<T> &pInputs) {
    inputs = pInputs;
    ...
    if (output.size() != inputs.size() || output[0].size() != inputs[0].size()) {
...
template <typename T>
void DropoutLayer<T>::backward(const Vec2d<T> &dValues) {
    if (dInputs.size() != dValues.size() || dInputs[0].size() != dValues[0].size()) {
        // avoids a copy in all but the first iteration
...
template <typename T>
Vec2d<T> DropoutLayer<T>::getOutput() {
    return output;
}

template <typename T>
Vec2d<T> DropoutLayer<T>::getDInputs() {
    return dInputs;
}
